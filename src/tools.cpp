...
rmarker Tools::radarSense(Car& car, Car ego, pcl::visualization::PCLVisualizer::Ptr& viewer, long long timestamp, bool visualize)
{
    ...
    double phi = std::atan2(car.position.y-ego.position.y,car.position.x-ego.position.x);
...
{
    UKF ukf = car.ukf;
    const Eigen::VectorXd& current_state = ukf.getState();
    viewer->addSphere(pcl::PointXYZ(current_state[0], current_state[1],3.5), 0.5, 0, 1, 0, car.name + "_ukf");
    viewer->addArrow(pcl::PointXYZ(current_state[0], current_state[1],3.5),
                     pcl::PointXYZ(current_state[0] + current_state[2]*cos(current_state[3]),
                                   current_state[1] + current_state[2]*sin(current_state[3]),3.5),
    ...
    if(time > 0)
    {
        ...
        {
            ukf.Prediction(dt);
            const Eigen::VectorXd& predicted_state = ukf.getState();
            viewer->addSphere(pcl::PointXYZ(predicted_state[0], predicted_state[1], 3.5), 0.5, 0, 1, 0,car.name+"_ukf"+std::to_string(ct));
